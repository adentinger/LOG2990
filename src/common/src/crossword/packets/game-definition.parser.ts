...
export class GameJoinParser extends PacketParser<GameDefinitionPacket> {
    public serialize(value: GameDefinitionPacket): ArrayBuffer {
        ...
        DATA.setInt32(0, DEFINITION_TEXT_LENGTH);
        for (let i = 0; i < DEFINITION_TEXT_LENGTH; i++) {
            DATA.setUint8(i + SIZE_UINT32, value.definition.text.charCodeAt(i));
        }
        return BUFFER;
    }
    ...
    public parse(data: ArrayBuffer): GameDefinitionPacket {
        ...
        const DEFINITION_TEXT_LENGTH = VIEW.getInt32(0);
        let buffer = '';
        for (let i = 0; i < DEFINITION_TEXT_LENGTH; i++) {
            buffer += String.fromCharCode(VIEW.getUint8(i + SIZE_UINT32));
        }
        return new GameDefinitionPacket(new Definition(buffer));
    }
}
